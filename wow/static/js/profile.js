...
var DynamicMenu = {
	...
	_populateList: function (categoryObj, entryId) {
		...
		if (DynamicMenu.cache.filtering) {
			...
			} else ah.resetSearch(DynamicMenu.cache.filtering);
			DynamicMenu.cache.filtering = null;
		}
			
		dm.cache.activeCategoryId = categoryId;
		
		$(".search-container:first, .container, .table").hide();

		if (categoryExists) {

			/* statistics */
			if (isParentCategory && !isOverview)
				$("#cat-" + categoryId).parent().find(".table").show();

			$("#cat-" + categoryId).show();
			$(".search-container:first").toggle((!isOverview));
	...
	openEntry: function (direct) {
		...
		var container = (requestedCategory == 92 || requestedCategory == 81 || requestedCategory == 15088 || requestedCategory == 15077 || requestedCategory == 15080 || requestedCategory == 15089 || requestedCategory == 15093) ? 'profile-sidebar-menu': 'swipe-in-container';
	...
	init: function() {
		$('#profile-sidebar-menu').delegate('li.disabled a', 'mouseover', function() {
			...
			if($this.hasClass('vault')) {
				...
				message = MsgProfile.tooltip.feature.notYetAvailable;
			}
